...
import { Search, Filter, Grid, List, ShoppingCart, Star, Eye, Plus, Package, Loader2, SortAsc, SortDesc } from 'lucide-react';
...
export default function StorePage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Fetch products
            const { response: productRes } = await AppService.productsPage.get({});
            if (productRes?.success && Array.isArray(productRes.data)) {
                ...
                console.log("productsWithMerchantNames", productsWithMerchantNames);
            ...
            // Fetch bundles
            const bundleRes = await AppService.packages.get({});
    ...
    const handleAddToAgencyStore = async (item: Product | Bundle, isBundle: boolean) => {
        if (!window.confirm(`Are you sure you want to add this ${isBundle ? 'bundle' : 'product'} to your agency store?`)) {
            return;
        }
        ...
        try {
            ...
            if (isBundle) {
                ...
                response = await AppService.productsPage.update({
                    payload: payload,
                });
            }
            ...
            if (response?.response?.success) {
                await fetchData();
                alert(`${isBundle ? 'Bundle' : 'Product'} added to agency store successfully!`);
            } else {
                alert(`Failed to add to agency store: ${response?.response?.message || 'Unknown error'}`);
            }
        } catch (error) {
            console.error('Error adding to agency store:', error);
...
useEffect(() => {
    const filterAndSort = (items: (Product | Bundle)[], isBundle = false) => {
        ...
        return items
            ...
            const allFilteredProducts = filterAndSort(products) as Product[];

        setFilteredProducts(allFilteredProducts.filter((p) => (p as Product).customizable == false) as Product[]);
    ...
    const ProductCard = ({ product }: { product: Product }) => {
        ...
        const price = firstVariant?.unitPrice || 0;
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-all duration-300 overflow-hidden group">
                <div className="relative aspect-square">
                    ...
                    {isAddedToAgency && (
                        <div className="absolute top-3 right-3">
                            <span className="bg-green-100 text-green-800 text-xs font-medium px-2 py-1 rounded-full">Added to your agency</span>
                        </div>
                    )}
                ...
                <div className="p-4">
                    ...
                    <div className="flex gap-2 mt-2">
                        <button
                            onClick={() => handleAddToAgencyStore(product, false)}
                            disabled={addingItemId === product._id}
                            className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-75"
                        >
        ...
        return (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-all duration-300 overflow-hidden group">
                ...
                <div className="p-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => navigate(`/app/${company_id}/${planId}/${userId}/store/custom-product/${product?.productId}`)}
                            className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center justify-center gap-2"
                        >
                            <Eye className="h-4 w-4" />
                            View Details
    ...
    const BundleCard = ({ bundle }: { bundle: Bundle }) => {
        // const savings = (bundle?.totalPrice || 0) - (bundle?.price || 0);
        // const savingsPercentage = bundle?.totalPrice ? Math.round((savings / bundle.totalPrice) * 100) : 0;
        const isAddedToAgency = bundle.companyId?.includes(company_id || '');
        ...
        return (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-all duration-300 overflow-hidden group">
                ...
                <div className="p-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => navigate(`/app/${company_id}/${planId}/${userId}/store/bundle/${bundle?.bundleId}`)}
                            className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center justify-center gap-2"
                        >
                            <Eye className="h-4 w-4" />
                            View Details
                    ...
                    <div className="flex gap-2 mt-2">
                        <button
                            onClick={() => handleAddToAgencyStore(bundle, true)}
                            disabled={addingItemId === bundle._id}
                            className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center justify-center gap-2 disabled:opacity-75"
                        >
                            {addingItemId === bundle._id ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                            ) : (
                                <>
                                    <Plus className="h-4 w-4" />
