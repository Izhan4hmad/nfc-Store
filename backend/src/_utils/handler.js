...
const ApiErrorHandler = (fn) => {
  return (req, res, next) => {
    fn(req, res, next).catch((err) => {
      // //console.log(req.originalUrl + ' - ERROR ', err.response?.data || err)
...
const ServiceHandler = async (fn, data, res) => {
  ...
  if (result?.headers) {
    // Set headers if present in the result
...
const ScriptServiceHandler = async (fn, data, res) => {
  const result = await fn(data);
  ...
  if (result?.success) {
    const { execution, ...detail } = result;
    res.status(enums.ResponseStatus.SUCCESS).send(detail.data);
    execution && execution();
    return;
  }
};
const HeaderServiceHandler = async (fn, data, res) => {
  const result = await fn(data);
...
const CssServiceHandler = async (fn, data, res) => {
  const result = await fn(data);
